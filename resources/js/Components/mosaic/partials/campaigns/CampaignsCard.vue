<template>
  <div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white shadow-lg rounded-sm border border-slate-200">
    <div class="flex flex-col h-full p-5">
      ...
      <div class="grow mt-2">
        <router-link class="inline-flex text-slate-800 hover:text-slate-900 mb-1" :to="item.link">
          <h2 class="text-xl leading-snug font-semibold">{{item.title}}</h2>
        </router-link>
        <div class="text-sm">{{item.content}}</div>
      </div>
      <footer class="mt-5">
        <div class="text-sm font-medium text-slate-500 mb-2">{{item.dates.from}} <span class="text-slate-400">-&gt;</span> {{item.dates.to}}</div>
